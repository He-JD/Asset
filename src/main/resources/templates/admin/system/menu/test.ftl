...
<fieldset class="layui-elem-field">
    ...
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal" data-type="addMenu">添加根菜单</a>
    </div>
    </div>
</fieldset>
    ...
    layui.use(['tree', 'layer','table'], function() {
        ...
        var layout = [
            ...
            },
            {
                ...
                render:function(row){
                    return undefined === row.icon?"" : '<i class="layui-icon" style="font-size: 25px;">'+row.icon+'</i>';
                }
            },
            {
                name: '排序',
        ...
        $(function(){
            $.get("${base}/admin/system/menu/tree",function(res){
                if(res.status==200){
    ...
    var delMenu =function(data){
        layer.confirm("你确定要删除该菜单么？这将会使得其下的所有子菜单都删除",{btn:['是的,我确定','我再想想']},
                function(){
                    ...
                    $.ajax({
                        url:"${base}/admin/system/menu/"+data,
                        type:'put',
                        dataType:'json',
